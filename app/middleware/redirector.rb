class Redirector
  ...
  def call(env)
    ...
    if request.host != HOST && request.path !~ %r{^/api}
      ...
      redirect_to("http://docs.getbpm.org#{request.path}")
    elsif request.path =~ %r{^/pages/docs$}
      redirect_to("http://guides.getbpm.org")
    elsif request.path =~ %r{^/pages/gem_docs$}
      redirect_to("http://guides.getbpm.org/command-reference")
    elsif request.path =~ %r{^/pages/api_docs$}
      redirect_to("http://guides.getbpm.org/rubygems-org-api")
    else
      @app.call(env)
    end
  end
