Feature: Push Gems
  ...
  Scenario: User pushes existing version of existing gem
    ...
    Then the response should contain "Repushing of package versions is not allowed."
    And the response should contain "Please use `bpm yank` to remove bad releases."
    And I visit the gem page for "PGem"
  ...
  Scenario: User pushes new version of existing gem
    Given I am signed up and confirmed as "email@person.com/password"gem
    And I have an API key for "email@person.com/password"
  ...
  Scenario: User pushes gem with bad runtime dependency
    ...
    Then I should see "Please specify dependencies that exist on GetBPM.org"
  ...
  Scenario: User pushes file that is not a gem
    ...
    Then I should see "GetBPM.org cannot process this package."
